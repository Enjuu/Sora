...
namespace Sora
{
    ...
    {
        public PresenceKeeper() => Values = new Dictionary<Token, Presence>(new TokenEqualityComparer());
        ...
        public void Push(IPacket packet, Presence skip = null)
        {
            try
            {
                ...
                foreach (var pr in Values.Where(pr => pr.Value != skip)) pr.Value.Push(packet);
            }
            finally
            {
